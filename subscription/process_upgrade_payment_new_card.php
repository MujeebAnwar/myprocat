<?php
// Start a UI payment (new card) for upgrading an existing subscription, and create a new Vault GUID.
...
$annualPaymentType = isset($_POST['annualPaymentType']) ? $_POST['annualPaymentType'] : 'one_time';
$monthlyPaymentAmount = isset($_POST['monthlyPaymentAmount']) ? floatval($_POST['monthlyPaymentAmount']) : 0.00;
$totalAnnualAmount = isset($_POST['totalAnnualAmount']) ? floatval($_POST['totalAnnualAmount']) : 0.00;
$installments = isset($_POST['installments']) ? intval($_POST['installments']) : 1;
$subscribed_plan_id = isset($_POST['subscribed_plan_id']) ? $_POST['subscribed_plan_id'] : '';

if ($amount <= 0 || $hours < 1 || empty($subscribed_plan_id)) {
...
if (
    !isset($sub[0]) || !is_array($sub[0]) || $sub[0]['id'] === 'id' ||
    $sub[0]['id_owner'] != $UserAccount->user_details['id_user'] || (int)$sub[0]['is_active'] !== 1
) {
    header('Location: /subscription/upgrade.php');
    exit;
}
...
$paymentTypeLabel = ($billing === 'annual' && $annualPaymentType === 'monthly' && $installments > 1) ? 'Monthly Installment 1/' . $installments : ucfirst($billing);
$order_number = $planName . " - " . $hours . " hours (" . $paymentTypeLabel . ")";

// Store context so the response handler knows this is an upgrade new-card flow
$data = json_encode([
    'flow' => 'upgrade_new_card',
...
$redirectUrl = $protocol . '://' . $host . $basePath . '/upgrade_payment_response.php';
?>
