...
class navbar extends content_block
{
	...
	public function __construct($user = NULL, $DB = NULL, $parameters = array())
	{
		...
		$logoImg = new image('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSOHTAekTL0-95EVIkVT_P3GW3_PDElWqr83A&s', array('style' => 'height: 50px;'));
		$logo->push($logoImg);
		$header->push($logo);
		...
		$navList->push(new content_block(
			new anchor('Plans', array('href' => '/subscription/payment.php')),
			'li'
		));

		$navList->push(new content_block(
			new anchor('Invoices', array('href' => '/subscription/invoices.php')),
			'li'
		));
		$navList->push(new content_block(
			new anchor('My Team', array('href' => '/account_view.php')),
			'li'
		));
		$navList->push(new content_block(
		...
		if($this->user && !is_null($this->user) && is_a($this->user, 'useraccount') && $this->user->logged_in)
		{
