...
// Page banner (renders above breadcrumbs in templateV2 mainframe)
$page_banner = new content_block(NULL, 'div', array('class' => 'banner'));
$page_banner->push(new content_block('Invoices', 'h1'));
...
if($UserAccount->user_details['is_admin']) {
    // Filter text input
...
$headerRow->push(new content_block('Payment Date', 'th', array('style' => 'border-color: rgb(234, 234, 234);')));
...
// Row 2
    ...
    $row2->push(new content_block(
        '<a href="#"
            style="color:#1a365d; font-weight:600; text-decoration:underline; cursor:pointer;"
            data-invoice-id="' . (int)$invoice['invoice_id'] . '"
            data-invoice-label="' . htmlspecialchars($invoice['invoice_number']) . '"
            onclick="return openViewInvoiceModal(this);"
        >#' . htmlspecialchars($invoice['invoice_number']) . '</a>',
        'td'
    ));
...
#viewInvoiceModalTitle{
    color: #27475f;
    font-size: 22px;
    margin: 0 40px 16px 0;
    text-align: left;
}
#viewInvoiceModalBody{
    overflow-y: auto;
    flex: 1;
}
', 'style');
...
$viewModalHtml = new content_block('
<div id="viewInvoiceModalOverlay" onclick="viewInvoiceModalOverlayClick(event)">
  <div id="viewInvoiceModal" class="form-box" role="dialog" aria-modal="true" aria-labelledby="viewInvoiceModalTitle">
    <button id="viewInvoiceModalClose" type="button" onclick="closeViewInvoiceModal()">×</button>
    <h2 id="viewInvoiceModalTitle">Invoice <span style="color:#666; font-size:14px;">#<span id="viewInvoiceModalInvoiceNumber"></span></span></h2>
    <div id="viewInvoiceModalBody"></div>
  </div>
</div>
', 'div');
$set_body->push($viewModalHtml);
...
function openViewInvoiceModal(el){
    try{
        var invoiceId = el.getAttribute("data-invoice-id") || "";
        var invoiceLabel = el.getAttribute("data-invoice-label") || invoiceId;
        var overlay = document.getElementById("viewInvoiceModalOverlay");
        var titleEl = document.getElementById("viewInvoiceModalInvoiceNumber");
        var bodyEl = document.getElementById("viewInvoiceModalBody");

        if(titleEl){ titleEl.textContent = invoiceLabel; }
        if(bodyEl){
            bodyEl.innerHTML = "<div style=\\"padding:24px;text-align:center;color:#666;\\">Loading invoice...</div>";
        }
        if(overlay){
            overlay.style.display = "flex";
        }
        ...
        fetch("/subscription/view_invoice.php?invoice_number=" + encodeURIComponent(invoiceId) + "&modal=1")
            ...
            .catch(function(){
                if(bodyEl){
                    bodyEl.innerHTML = "<div style=\\"padding:24px;text-align:center;color:#dc3545;\\">Unable to load invoice.</div>";
                }
            });

        document.addEventListener("keydown", viewInvoiceModalEscClose);
    }catch(e){}
    return false;
}
...
function closeViewInvoiceModal(){
    ...
    document.removeEventListener("keydown", viewInvoiceModalEscClose);
}

function viewInvoiceModalEscClose(e){
    if(e && (e.key === "Escape" || e.keyCode === 27)){
        closeViewInvoiceModal();
    }
}

function viewInvoiceModalOverlayClick(e){
    var overlay = document.getElementById("viewInvoiceModalOverlay");
    if(e && overlay && e.target === overlay){
        closeViewInvoiceModal();
    }
